...
const GC = (selector) => {
    ...
    function draw_text(elm, x, y, text, color, size, line_color, line_size, font, pattern, textAlign, direction, style = "", LetterSpacing, BaseAlign) {
        GC_data[elm]["context"].beginPath();
    ...
    function get_text_range(elm, x, y, text, color, size, line_color, line_size, font, pattern, textAlign, direction, style = "", LetterSpacing, BaseAlign) {
        let pos = get_position(elm, x, y, null, null, line_size, size);
        GC_data[elm]["context"].font = `${style} ${pos.size}pt ${font}`;
        GC_data[elm]["context"].textAlign = textAlign;
        GC_data[elm]["context"].textBaseline = BaseAlign;
    ...
    return {
        ...
        draw: {
            ...
            bow: function (x, y, rotate, color, size, pattern) {
                elm.forEach(elm => {
                    const ctx = GC_data[elm]["context"];
                    const canvasWidth = GC_data[elm].elm.width;
                    const canvasHeight = GC_data[elm].elm.height;
                    ctx.beginPath();
                    GC_data.bag_num++;
                    GC_data.s_r_history.push("Plus-bow");
                    ctx.save();
                    set_pattern_and_color(elm, pattern, [null, color]);
                    let pos = get_position(elm, x, y, 0, 0, null, size);
                    ...
                    if (GC_data["is_loaded_image"][img_url]) {
                        if (size.keep_origin) {
                            if (GC_data["image"][img_url].width > GC_data["image"][img_url].height) {
                                width = height * (elm.height / elm.width);
                            } else {
                                height = width * (elm.width / elm.height);
                            }
                            x -= (width / 2);
                            y -= (height / 2);
                        }
                    ...
                    for (let i = 0; i < de_text.length; i++) {
                        ...
                        if (Turn_around != Infinity) {
                            for (let g = 0; g < de_text[i].length; g++) {
                                if (Un_transform_position(elm, get_text_range(elm, x, y, de_text[i].slice(ni, g), color, temp_size, line_color, line_size, font, pattern, textAlign, direction, style, LetterSpacing, BaseAlign).dx, null, null).x > Turn_around || g == (de_text[i].length - 1)) {
                                    let show_text = de_text[i].slice(ni, g);
                                    if (g == (de_text[i].length - 1)) {
                                        show_text = de_text[i].slice(ni);
                                    }
                                    let left = Un_transform_position(elm, get_text_range(elm, x, y, show_text, color, temp_size, line_color, line_size, font, pattern, textAlign, direction, style, LetterSpacing, BaseAlign).x, null, null).x;
                                    let right = Un_transform_position(elm, get_text_range(elm, x, y, show_text, color, temp_size, line_color, line_size, font, pattern, textAlign, direction, style, LetterSpacing, BaseAlign).dx, null, null).x;
                                    let width = right - left;
                                    let top = Un_transform_position(elm, null, get_text_range(elm, x, y, show_text, color, temp_size, line_color, line_size, font, pattern, textAlign, direction, style, LetterSpacing, BaseAlign).y, null).y;
                                    let bottom = Un_transform_position(elm, null, get_text_range(elm, x, y, show_text, color, temp_size, line_color, line_size, font, pattern, textAlign, direction, style, LetterSpacing, BaseAlign).dy, null).y;
                        ...
                        for (let i = 0; i < Confirm_text.length; i++) {
                            temp_num = Un_transform_position(elm, draw_text(elm, x + lasted_position, (y + ((Confirm_text[i].dy + Ly) * i) - RI_af), Confirm_text[i].text, color, temp_size, line_color, line_size, font, pattern, textAlign, direction, style, LetterSpacing, BaseAlign), null, null).x;
                            Ly = Line_spacing;
                        }
            ...
            click: function (function_name) {
                elm.forEach(elm => {
                    elm.addEventListener("click", function (e) {
                        ...
                        let pos = Un_transform_position(elm, (e.clientX - clientRect.left) * GC_data[elm].quality, (e.clientY - clientRect.top) * GC_data[elm].quality);
            ...
            move: function (function_name) {
                elm.forEach(elm => {
                    elm.addEventListener("touchmove", function (e) {
                        var clientRect = elm.getBoundingClientRect();
                        let pos = Un_transform_position(elm, (e.touches[0].pageX - clientRect.left) * GC_data[elm].quality, (e.touches[0].pageY - clientRect.top) * GC_data[elm].quality);
                        setTimeout(function_name, 0, pos.x, pos.y);
                    })
            ...
            clicking: function (functions) {
                elm.forEach(elm => {
                    ...
                    elm.addEventListener("touchstart", function (e) {
                        var clientRect = elm.getBoundingClientRect();
                        let pos1 = Un_transform_position(elm, (e.touches[0].pageX - clientRect.left) * GC_data[elm].quality, (e.touches[0].pageY - clientRect.top) * GC_data[elm].quality);
                    ...
                    elm.addEventListener("mousedown", function (e) {
                        var clientRect = elm.getBoundingClientRect();
                        let pos1 = Un_transform_position(elm, (e.clientX - clientRect.left) * GC_data[elm].quality, (e.clientY - clientRect.top) * GC_data[elm].quality);
                        GC_data.clicking_judge_box_position_2[i] = ([pos1.x, pos1.y]);
                        GC_data.clicking_judge_box[i] = true;
                    })
