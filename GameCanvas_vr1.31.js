...
const GC = (selector) => {
    ...
    function Un_transform_position(elm, x = 0, y = 0, size = 0) {
        ...
        if (GC_data[elm]["Calculation_method"] == "px") {
            ...
            return_pos["x"] = elm.width / x * 100;
            return_pos["y"] = elm.height / y * 100;
            return_pos["size"] = (elm.width + elm.height) / size * 200;
        } else if (GC_data[elm]["Calculation_method"] == "vmin") {
            return_pos["x"] = Math.vmin(elm.width, elm.height) / x * 100;
            return_pos["y"] = Math.vmin(elm.width, elm.height) / y * 100;
            return_pos["size"] = Math.vmin(elm.width, elm.height) / size * 100;
        } else if (GC_data[elm]["Calculation_method"] == "vmax") {
            return_pos["x"] = Math.vmax(elm.width, elm.height) / x * 100;
            return_pos["y"] = Math.vmax(elm.width, elm.height) / y * 100;
            return_pos["size"] = Math.vmax(elm.width, elm.height) / size * 100;
        };
        return return_pos;
    }
    function set_translate(elm, x, y, dx, dy, x3, y3) {
        ...
        if (GC_data[elm]["debug"] != null) {
            if (GC_data[elm]["debug"]) {
                ...
                if (GC_data[elm]["debug"]["line_width"] != null) {
                    GC_data[elm]["context"].lineWidth = GC_data[elm]["debug"]["line_width"];
                }
